...
router.get('/news_detail/:news_id',(req,res)=>{
  (async function(){
    ...
    if(userInfo[0]){  // 如果已经登录
      // 查询数据库
      let collectionResult = await handleDB(res, "info_user_collection", "find", "查询数据库出错",
        `user_id=${userInfo[0].id} and news_id=${news_id}`
      )  // [{}]    []
      if(collectionResult[0]){   //收藏了这篇新闻
        isCollected = true;
      }
    }

    // 查询和这篇新闻相关的评论，[{}, {}, {} ...], 以创建时间降序排序
    let commentResult = await handleDB(res, "info_comment", "find", "查询数据库出错",
    ...
    //把登录用户的点赞过的评论全部查出来 ,传给前端模板
    let user_like_comments_ids =[];
    if(userInfo[0]){


    //  查询up莽荒纪的点赞过的评论
      let user_like_comments_Result =
         await handleDB(res, "info_comment_like", "find", "数据库查询失败",
           `user_id=${userInfo[0].id}`);

      user_like_comments_Result.forEach(el=>{
    ...
    //查询新闻作者信息
    let authorResult =
      await handleDB(res, "info_user", "find", "数据库查询失败",
        `id=${newsResult[0].user_id}`)

    // console.log(authorResult)
  let Id = newsResult[0].id?newsResult[0].id:'6'
    let authorNewsCount =
      await handleDB(res, "info_news", "sql", "数据库查询失败",
        `select count(*) from info_news where id=${Id}`);
      // 传到模板中去的数据
    let data = {
      user_info:userInfo[0]?{
        nick_name: userInfo[0].nick_name,
        avatar_url: userInfo[0].avatar_url?userInfo[0].avatar_url:"../../images/worm.jpg"
      }:false,
      newsClick:result3,
  ...
  (async function(){
    ...
    */
    ...
    if(action ==="collect"){
      ...
      await handleDB(res, "info_user_collection", "delete", "数据库添加失败",
        `user_id=${userInfo[0].id} and news_id=${news_id}`
      )
    }
    //    5、返回操作成功
...
router.post('/news_detail/comment_like',(req,res)=>{
  (async function () {
    ...
    if(action === "add"){
    //  点赞
      await handleDB(res, "info_comment_like", "insert", "数据库插入出错",
        {
          comment_id,
          user_id:userInfo[0].id
        });
    ...
    //  取消点赞
      await handleDB(res, "info_comment_like", "delete", "数据库插入出错",
        `comment_id=${comment_id} and user_id=${userInfo[0].id}`)
      var like_count = commentResult[0].like_count?commentResult[0].like_count-1: 0;

    }
    await handleDB(res, "info_comment", "update", "数据库插入出错",
      `id=${comment_id}`,{like_count})
